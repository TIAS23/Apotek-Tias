...
if ($data !== null) {
    echo '<div class="h5 mb-0 font-weight-bold text-gray-800">Rp. ' . number_format($data->total) . '</div>';
} else {
...
$tgl = date('m');
$thn = date('Y');
$stmt = $connect->prepare("SELECT COUNT(kode) AS total FROM tb_obat WHERE MONTH(expired) = ? AND YEAR(expired) = ?");
$stmt->bind_param("ss", $tgl, $thn);
$stmt->execute();
$result = $stmt->get_result();
$row = $result->fetch_assoc();
$total = $row['total'];

echo '<div class="h5 mb-0 font-weight-bold text-gray-800">' . number_format($total) . ' Data</div>';
?>
      ...
      <div class="card-body">
        ...
        <script>
          ...
          var myChart = new Chart(ctx, {
            ...
            type: 'line',
            data: {
              labels: [<?php while ($b = $bulan->fetch_object()) {
                          echo '"' . bulan($b->bulan) . '",';
                        } ?>],
              datasets: [{
                ...
                data: [<?php while ($p = $penghasilan->fetch_object()) {
...
<script>
  ...
  var myChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: [<?php while ($b = $nama->fetch_object()) {
                  echo '"' . bulan($b->namaob) . '",';
                } ?>],
      ...
      datasets: [{
        label: 'Total Penjualan',
        data: [<?php while ($p = $count->fetch_object()) {
                  echo '"' . $p->jumlah_jual . '",';
                } ?>],
